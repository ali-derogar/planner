...
function taskCard(t) {
    ...
    if (t.estimated > 0) {
        ...
        if (t.remaining_fmt) {
            progress += '<div class="task-remaining">?????: ' + esc(t.remaining_fmt) + '</div>';
        }
    }
    ...
    if (t.is_expanded) {
        var timerBtn = t.is_running
            ? '<button class="btn-stop" onclick="action(\'stop_timer\',{id:' + t.id + '})">? ????</button>'
            : '<button class="btn-start" onclick="action(\'start_timer\',{id:' + t.id + '})">? ????</button>';
        ...
        detail = '<div class="task-detail">' +
            ...
            '<div class="est-row"><span>?????: ' + esc(t.estimated_fmt) + '</span>' +
            '<a href="javascript:void(0)" onclick="showModal({title:\'????? ?????\',cmd:\'set_estimated\',params:{id:' + t.id + '},fields:[{label:\'???\',key:\'value\',placeholder:\'00:00:00\',validate:\'hms\'}]})" class="chip chip-edit">?????</a></div>' +
            '<div class="chips">' +
            '<a href="javascript:void(0)" onclick="action(\'set_useful\',{id:' + t.id + ',value:\'true\'})" class="chip ' + uCls + '">? ????</a>' +
            '<a href="javascript:void(0)" onclick="action(\'set_useful\',{id:' + t.id + ',value:\'false\'})" class="chip ' + nuCls + '">? ???????</a>' +
            '<a href="javascript:void(0)" onclick="showModal({title:\'?????? ?????\',cmd:\'edit_title\',params:{id:' + t.id + '},fields:[{label:\'?????\',key:\'value\',value:\'' + escJs(t.title) + '\',validate:\'required\'}]})" class="chip chip-edit">? ?????</a>' +
            '<a href="javascript:void(0)" onclick="showModal({title:\'?????? ???\',cmd:\'set_duration\',params:{id:' + t.id + '},fields:[{label:\'???\',key:\'value\',validate:\'hms\'}]})" class="chip chip-edit">? ???</a>' +
            '<a href="javascript:void(0)" onclick="action(\'copy_task\',{id:' + t.id + '})" class="chip chip-edit">?? ????</a>' +
            ...
            '<a href="javascript:void(0)" onclick="action(\'delete_task\',{id:' + t.id + '})" class="chip chip-delete">??</a>' +
            '</div></div>';
    }
...
function renderHome(h) {
    ...
    var recurBtn = '<button class="icon-btn" onclick="action(\'navigate\',{screen:\'recurring\'})" title="??????">? ' + pd(h.recurring_count) + '</button>';
    ...
    var html = '<div class="date-header">' +
        ...
        (h.is_today ? '' : '<a href="javascript:void(0)" onclick="action(\'today\')" class="today-btn">?????</a>') +
    ...
    html += '<div class="summary-bar">' +
        '<span class="sum-useful">????: ' + esc(h.useful_fmt) + '</span>' +
        '<span class="sum-eff">??????: ' + pd(h.efficiency) + '?</span>' +
        '<span class="sum-not">???????: ' + esc(h.not_useful_fmt) + '</span></div>';

    html += '<div class="search-row"><input class="search-input" placeholder="?????..." value="' + esc(h.search) + '" oninput="debounceSearch(this.value)" /></div>';

    html += '<div class="task-list">';
    if (h.tasks.length === 0) {
        html += '<div class="empty-state"><div class="empty-icon">??</div><div>???? ???? ?????</div>' +
            '<button class="empty-btn" onclick="showModal({title:\'?????? ???\',cmd:\'add_task\',fields:[{label:\'?????\',key:\'title\',validate:\'required\'}]})">+ ?????? ???</button></div>';
    } else {
    ...
    html += '<div class="section note-section"><div class="sec-title">??????? ??? ??</div>' +
        '<textarea class="note-input" placeholder="??????? ?????..." onchange="action(\'set_note\',{value:this.value})">' + esc(h.daily_note) + '</textarea></div>';
    html += '<a href="javascript:void(0)" onclick="showModal({title:\'?????? ???\',cmd:\'add_task\',fields:[{label:\'?????\',key:\'title\',validate:\'required\'}]})" class="add-btn">+ ?????? ???</a>';
    return html;
}

function renderCalendar(cal) {
    var html = '<div class="calendar-panel"><div class="cal-header">' +
        '<button class="cal-nav" onclick="action(\'cal_prev_month\')">?</button>' +
        '<span>' + esc(cal.month_name) + ' ' + pd(cal.year) + '</span>' +
        '<button class="cal-nav" onclick="action(\'cal_next_month\')">?</button></div><div class="cal-grid">';
...
function renderFinance(fin) {
    var rows = fin.entries.map(function(e) {
        ...
        return '<div class="fin-entry">' +
            ...
            '<button class="fin-edit" onclick="showModal({title:\'??????\',cmd:\'edit_finance\',params:{id:' + e.id + '},fields:[{label:\'?????\',key:\'title\',value:\'' + escJs(e.title) + '\',validate:\'required\'},{label:\'????\',key:\'amount\',value:\'' + e.amount + '\',type:\'number\',validate:\'amount\'},{label:\'????\',key:\'category\',type:\'select\',value:\'' + escJs(e.category) + '\',options:window._categories}]})">?</button>' +
            '<button class="fin-del" onclick="action(\'delete_finance\',{id:' + e.id + '})">?</button></div>';
    }).join('');
    if (!rows) rows = '<div class="empty-mini">???? ????? ????</div>';
    var balCls = fin.balance >= 0 ? 'fin-income' : 'fin-expense';
    return '<div class="section"><div class="sec-header"><span class="sec-title">???? ??</span><div class="sec-actions">' +
        '<a href="javascript:void(0)" onclick="showAddFinance(\'income\')" class="btn-sm-green">+ ?????</a>' +
        '<a href="javascript:void(0)" onclick="showAddFinance(\'expense\')" class="btn-sm-red">+ ?????</a></div></div>' +
        ...
        '<div class="fin-summary"><span class="fin-income">?????: ' + esc(fin.income_fmt) + '</span>' +
        '<span class="' + balCls + '">?????: ' + esc(fin.balance_fmt) + '</span>' +
        '<span class="fin-expense">?????: ' + esc(fin.expense_fmt) + '</span></div></div>' + rows + '</div>';
}
...
function showAddFinance(type) {
    showModal({
        ...
        params: { type: type },
        fields: [
            { label: '?????', key: 'title', validate: 'required' },
            { label: '???? (?????)', key: 'amount', type: 'number', validate: 'amount' },
            { label: '????', key: 'category', type: 'select', value: '?????', options: window._categories },
        ],
    });
}
...
function renderWellness(w) {
    ...
    return '<div class="section"><div class="sec-header"><span class="sec-title">????? ??</span></div>' +
        '<div class="well-row">' +
        '<a href="javascript:void(0)" onclick="showModal({title:\'???? ????\',cmd:\'set_sleep\',fields:[{label:\'HH:MM\',key:\'value\',value:\'' + w.sleep_raw + '\',validate:\'hm\'}]})" class="well-btn"><span class="well-lbl">????</span><span class="well-val">' + esc(w.sleep) + '</span></a>' +
        '<a href="javascript:void(0)" onclick="showModal({title:\'???? ??????\',cmd:\'set_wake\',fields:[{label:\'HH:MM\',key:\'value\',value:\'' + w.wake_raw + '\',validate:\'hm\'}]})" class="well-btn"><span class="well-lbl">??????</span><span class="well-val">' + esc(w.wake) + '</span></a>' +
        '<div class="well-btn well-static"><span class="well-lbl">??? ????</span><span class="well-val">' + esc(w.sleep_dur) + '</span></div></div>' +
...
function renderAnalytics(a) {
    ...
    var s = a.stats;
    var html = '<div class="analytics-header sticky-sub"><span class="analytics-title">?? ??????</span></div>' +
        '<div class="analytics-period">' +
        '<a href="javascript:void(0)" onclick="action(\'set_period\',{days:7})" class="' + p7 + '">? ???</a>' +
        '<a href="javascript:void(0)" onclick="action(\'set_period\',{days:30})" class="' + p30 + '">?? ???</a></div>' +
        '<div class="period-label">' + esc(a.start_label) + ' ?? ' + esc(a.end_label) + '</div>' +
        '<div class="streak-badge">?? ' + pd(s.streak) + ' ??? ??????</div>' +
        '<div class="chart-box">' + sparklineSvg(a.chart_points, 300, 60) + '</div>' +
        heatmapHtml(a.heatmap) +
        '<div class="stat-cards">' +
        statCard('?? ????', s.total_fmt) +
        statCard('????', s.useful_fmt, '#4DD980') +
        statCard('???????', s.not_useful_fmt, '#FF7359') +
        statCard('??????', pd(s.eff) + '?', '#5E5CE6') +
        statCard('?????', s.income_fmt, '#4DD980') +
        statCard('?????', s.expense_fmt, '#FF7359') +
        statCard('?????', s.balance_fmt, '#5E5CE6') +
        statCard('????', s.avg_mood) +
        statCard('????', s.avg_sleep) +
        '</div><div class="sec-title" style="padding:8px 12px">?????? ??????</div>';
    ...
    if (!a.days.length) {
        ...
        a.days.forEach(function(d) {
            html += '<div class="day-card"><div class="day-date">' + esc(d.date_label) + '</div>' +
                ...
                '<div class="day-stats"><span>??: ' + esc(d.total_fmt) + '</span>' +
                '<span style="color:#4DD980">????: ' + esc(d.useful_fmt) + '</span>' +
                '<span style="color:#5E5CE6">??????: ' + pd(d.eff) + '?</span></div>' +
...
function renderSettings(s) {
    ...
    return '<div class="page-header">? ???????</div>' +
        '<div class="section"><div class="setting-row"><span>?? ????</span>' +
        '<button class="toggle-btn' + (dark ? ' on' : '') + '" onclick="action(\'set_theme\',{theme:\'' + (dark ? 'light' : 'dark') + '\'})">' +
        (dark ? '????' : '???????') + '</button></div></div>' +
        '<div class="section"><button class="add-btn" onclick="action(\'export_data\')">?? ????? JSON</button>' +
        '<p class="hint">??????? ?? ????????? WebView ????? ???????</p></div>' +
        '<button class="back-btn" onclick="action(\'navigate\',{screen:\'home\'})">? ??????</button>';
}

function renderRecurring(list) {
    var rows = list.map(function(r) {
        return '<div class="recur-row"><span>' + esc(r.title) + '</span>' +
            '<button class="chip chip-delete" onclick="action(\'delete_recurring\',{id:' + r.id + '})">???</button></div>';
    }).join('');
    if (!rows) rows = '<div class="empty-mini">???? ??? ??????</div>';
    return '<div class="page-header">? ??????? ??????</div><div class="section">' + rows + '</div>' +
        '<button class="back-btn" onclick="action(\'navigate\',{screen:\'home\'})">? ??????</button>';
}
...
function renderNav(screen) {
    ...
    return '<a href="javascript:void(0)" onclick="action(\'navigate\',{screen:\'home\'})" class="' + home + '"><span class="nav-icon">??</span>????</a>' +
        '<a href="javascript:void(0)" onclick="action(\'navigate\',{screen:\'analytics\'})" class="' + anal + '"><span class="nav-icon">??</span>??????</a>';
}
